...
/ Title: a4-rpk
/
/ Description: robotprotectskitten (classic) for Neo
/
/ Author:
/
/ Created:
...
void init(void) {
  ...
  scanf("%lu",&num);
  while(--num) r = neo_math_random_integer(10);
...
void render_board() {
  ...
  draw_rknki(kitten);
  draw_rknki(robot);
  for(uint8_t i=0;i<N_NKI;i++) if (nki[i].alive) draw_rknki(nki[i]);
  }
...
void rpk(void) { // robot protects kitten!
  clear_status();
  sprintf(g_status,"robot protects kitten! <3 <3 <3");
  neo_sound_play_effect(0,0);
  update_status();
  wait_ticks(200);
  
  char tmpstat[g_scw];
  clear_status();
  sprintf(g_status,"R       pp       K           ");
  update_status();
...
void rftpk(void) { // robot fails to protect kitten!
  clear_status();
  sprintf(g_status,"kitten is whacked by NKI. :( :( :(");
  neo_sound_play_effect(0,1);
  update_status();
  wait_ticks(200);
  }
...
void rfnki(int inki) {
  clear_status();
  sprintf(g_status,"NKI %d (%s) eliminated.\n",inki,nki[inki].desc);
  sprintf(g_status,"%s",nki[inki].desc);
  nki[inki].alive = FALSE;
  update_status();
  }
...
uint8_t handle_controller() {
  uint8_t dpad = DPAD(neo_controller_read(neo_controller_count()));
  if (!dpad) return TRUE;
...
// returns TRUE if NKI clobbers kitten, FALSE if not

int movenkis() {
  for(uint8_t i=0;i<N_NKI;i++)
    if (nki[i].alive) {
      int8_t dx = kitten.x - nki[i].x;
      int8_t dy = kitten.y - nki[i].y;
      if ((!dx) && (!dy)) { /*printf("bonk2");*/ return TRUE; }
      if (abs(dx) > abs(dy)) 
        nki[i].x += (dx>0) ? 1 : -1;
      else
        nki[i].y += (dy>0) ? 1 : -1;
      /*printf("%d: %d %d k %d %d \n",i,nki[i].x,nki[i].y,kitten.x,kitten.y);*/
      if ((nki[i].x == kitten.x) && (nki[i].y == kitten.y)) {
        /*printf("bonk.");*/return TRUE;
        }
      }
  return FALSE;
  }
...
int main(void) {
  ...
  while (handle_controller()) {
    // simple alternating bit makes nkis move at half the speed of the robot.
    if (neo_system_timer() % NKIPACE == 0) {
      if (movenkis()) {
        rftpk();
        break;
        }
