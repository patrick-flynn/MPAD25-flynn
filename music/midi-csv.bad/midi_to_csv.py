	...
	for file in os.listdir(input_dir):
		...
		for note in s.recurse().notes: 
			if note.isNote:
				# print(note.offset)
				# print(note.duration)
				new_df = pd.DataFrame([[note.pitch, int(100*round(float(note.offset), 3)), int(100*round(music21.common.numberTools.numToIntOrFloat(note.duration.quarterLength), 3)), note.volume.velocity, note_tempo]], columns=["note_name", "start_time", "duration", "velocity", "tempo"])
