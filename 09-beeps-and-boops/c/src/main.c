...
void wait_silence(uint8_t channel) {
  while (neo_sound_status(channel)) /* wait */ ;
  }
...
int main(int argc,char *argv[]) {
  ...
  while (1==1) {
    // make sure sound has finished
    wait_silence(0);
    // beep!
    neo_sound_play_effect(0,(snd_idx++)%N_SOUND_EFFECTS);
    for(i=0;i<STEPS;i++) {
