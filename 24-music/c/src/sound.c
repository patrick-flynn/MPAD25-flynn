...
void neo_sound_queue_extended(uint8_t channel, uint16_t frequency, uint16_t duration, uint16_t slide, uint8_t type, uint8_t volume) {
    ControlPort.params[0] = channel;
    *((volatile uint16_t *)(ControlPort.params+1)) = frequency;
    *((volatile uint16_t*) (ControlPort.params + 3)) = duration;
    *((volatile uint16_t*) (ControlPort.params + 5)) = slide;
    ControlPort.params[7] = type;
    *((volatile uint8_t*)(ControlPort.params+8)) = volume;
    KSendMessage(API_GROUP_SOUND, API_FN_QUEUE_SOUND_EX);
}
